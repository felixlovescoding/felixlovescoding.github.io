  ...
  const windowLoaded = () => {
    ...
    mainElem.on('click', e => {
      if ($(e.target).closest('.project-item').length > 0) {
        ...
        const projectGithubRepo = $(e.target).closest('.project-item').data('github');
      const projectLivePreview = $(e.target).closest('.project-item').data('live-preview');

      // Set the links for the popUp
      $(popUpElem)
        .children('#popUpText')
        .children('div')
        .children('#githubLink')
